            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700 p-8">
                ...
                <form action="{{ route('address_shippings.update', $address_shipping) }}" method="POST" id="form-edit-{{ $address_shipping->id }}" onsubmit="confirmarActualizacion(event, {{ $address_shipping->id }})" novalidate>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="municipality" name="municipality" value="{{ old('municipality', $address_shipping->municipality) }}" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                        @error('municipality')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        <label for="exempt_address" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Dirección Exacta</label>
                        <input type="text" id="exempt_address" name="exempt_address" value="{{ old('exempt_address', $address_shipping->exempt_address) }}" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
