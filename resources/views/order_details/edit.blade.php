            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700 p-8">
                ...
                <form action="{{ route('orders.update', $order) }}" method="POST" id="form-edit-{{ $order->id }}" onsubmit="confirmarActualizacion(event, '{{ $order->id }}')" novalidate>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="datetime-local" id="date_time" name="date_time" value="{{ old('date_time', $order->date_time ? \Carbon\Carbon::parse($order->date_time)->format('Y-m-d\TH:i') : '') }}" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                        @error('date_time')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
